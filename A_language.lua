...
-- Cayo Cut
    ...
    Cayo_Cut_Pavel_list   = {"Remove Pavel's cut","Enlever La part dePavel"}
    Cayo_Cut_Pavel        = Cayo_Cut_Pavel_list[current_language]
    Cayo_Fencing_Fee_list = {"Remove Fencing's fee","Enlever les Frais du porteur"}
...
-- Settings
    ...
    Boost_Key_list              = {"Boost key","Bouton de Boost"}
    Boost_Key                   = Boost_Key_list[current_language]
    Settings_Boost_Default_list = {"Enable boost button by default?","Activer le Bouton de Boost"}
    Settings_Boost_Default      = Settings_Boost_Default_list[current_language]
    Refill_Key_list             = {"Refill Inventory","Remplir l'inventaire"}
    Refill_Key                  = Refill_Key_list[current_language]
    -- Numberplates
...
-- Main menu
    ...
    -- Other's cool ppl script (modified)
        colorStyles    = {}
        colorStyles_1  = {"Rainbow","Arc-en-ciel"}
        colorStyles_2  = {"Strobelight","Clignotement"}
        colorStyles_3  = {"Random","Aléatiore"}
        colorStyles[1] = colorStyles_1[current_language]
        colorStyles[2] = colorStyles_2[current_language]
        colorStyles[3] = colorStyles_3[current_language]
        -- Boost Go Brrrrr
            Boost_Menu_list     = {"Boost menu","Boost de voiture (NOS)"}
            Boost_Menu          = Boost_Menu_list[current_language]
            Boost_Toggle_list   = {"Boost on press ?","Booster avec touche"}
            Boost_Toggle        = Boost_Toggle_list[current_language]
            Boost_Strength_list = {"Car Boost strength","Force du Boost"}
            Boost_Strength      = Boost_Strength_list[current_language]
        -- Gay Car
            Rainbow_Menu_list    = {"Rainbow menu (You Only)","Couleur Voiture Custom (Seul Toi)"}
            Rainbow_Menu         = Rainbow_Menu_list[current_language]
            Rainbow_Toggle_list  = {"Activate Rainbow Car","Activer les effets"}
            Rainbow_Toggle       = Rainbow_Toggle_list[current_language]
            Rainbow_Style_list   = {"Style →","Style →"}
            Rainbow_Style        = Rainbow_Style_list[current_language]
            Rainbow_Uniform_list = {"Uniform color","Couleur uniforme"}
            Rainbow_Uniform      = Rainbow_Uniform_list[current_language]
            Rainbow_Traffic_list = {"Affect traffic","Affecter le traffic"}
            Rainbow_Traffic      = Rainbow_Traffic_list[current_language]
            Rainbow_Mul_list     = {"Rainbow Speed Multiplier","Multiplicateur de vitesse"}
            Rainbow_Mul          = Rainbow_Mul_list[current_language]
    -- Other
        Menu_NoScratch_list           = {"No scratch","Pas d'égratinure"}
        Menu_NoScratch                = Menu_NoScratch_list[current_language]
        Menu_Disable_AutoStop_list    = {"Disable engine auto stop","Désactivez l'arret auto du moteur"}
        Menu_Disable_AutoStop         = Menu_Disable_AutoStop_list[current_language]
        Menu_LSC_Reset_list           = {"Reset LSC vehicle sell limit","Reinitialisez la limite de vente pour LSC"}
        Menu_LSC_Reset                = Menu_LSC_Reset_list[current_language]
        ...
        -- Stat List / Unlocks / Tunable
            Unlock_Tunable_Menu_list   = {"Unlocks/Tunable menu","Menu de débloquage"}
            Unlock_Tunable_Menu        = Unlock_Tunable_Menu_list[current_language]
            Achivement_Menu_list       = {"Achievements","Succés"}
            Achivement_Menu            = Achivement_Menu_list[current_language]
            -- Unlocks
                Unlocks_Daily_list     = {"Unlock Daily Objectives Awards","Débloquer les recompenses des objectifs"}
                Unlocks_Daily          = Unlocks_Daily_list[current_language]
                Unlocks_Parachute_list = {"Unlock Rare Parachutes","Débloquer les parachutes"}
                Unlocks_Parachute      = Unlocks_Parachute_list[current_language]
                Unlocks_Unicorn_list   = {"Unlock Vanilla Unicorn","Débloquer Vanilla Unicorn"}
            ...
            -- Stats
                ...
                Stats_Driving_list  = {"Driving"      ,"Conduite"}
                Stats_Flying_list   = {"Flying"       ,"Vol"}
                Stats_Shooting_list = {"Shooting"     ,"Tir"}
                Stats_Stealth_list  = {"Stealth"      ,"Discretion"}
                Stats_Mechanic_list = {"Mechanic"     ,"Méchanique"}
                Stats_Stamina       = Stats_Stamina_list[current_language]
                Stats_Strength      = Stats_Strength_list[current_language]
                Stats_Lung          = Stats_Lung_list[current_language]
                Stats_Driving       = Stats_Driving_list[current_language]
                Stats_Flying        = Stats_Flying_list[current_language]
                Stats_Shooting      = Stats_Shooting_list[current_language]
                Stats_Stealth       = Stats_Stealth_list[current_language]
                Stats_Mechanic      = Stats_Mechanic_list[current_language]
            -- Report
                Report_List = {}
